...
async def test_get_exchange_keys_success():
    ...
    with patch('src.data.exchange_keys.db.api_keys.find', AsyncMock(return_value = mock_api_keys)):
...
async def test_get_exchange_keys_failure():
    with patch('src.data.exchange_keys.server_info',side_effect = ConnectionError("MongoDB connection failed")):
        result = await get_exchange_keys()
        assert len(result) == 1
        assert result[0].exchange_name == 'Kraken'
